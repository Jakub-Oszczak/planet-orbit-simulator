from ast import ExtSlice
from re import S
from time import sleep
from turtle import distance, up
import pygame as pg
from math import *

pg.init()
WIDTH, HEIGHT = 800, 800
WHITE = (255, 255, 255)
YELLOW = (222, 148, 0)
BLUE = (58, 98, 138)
RED = (153, 29, 20)
GREYISH = (138, 132, 150)
GOLD = (138, 132, 150)
FONT = pg.font.SysFont("comicsans", 16)
win = pg.display.set_mode((WIDTH, HEIGHT))
pg.display.set_caption("Planets orbit simulator")

class Planet:
    AU = 149.6e9 # astronomical unit in meters
    G = 6.67428e-11 # gravitational constant 
    SCALE = 250 / AU # 1AU = 100 pixels
    TIMESTEP = 3600*24 # 1 day

    def __init__(self, x, y, radius, color, mass, y_vel):
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
        self.mass = mass
        self.x_vel = 0
        self.y_vel = y_vel
        self.sun = False
        self.distance_to_sun = 0
        self.orbit = []
    
    def draw(self, win):
        x = self.x * self.SCALE + WIDTH/2
        y = self.y * self.SCALE + HEIGHT/2

        if len(self.orbit) > 2:
            updated_orbit = []
            for point in self.orbit:
                x1, y1 = point
                x1 = x1 * self.SCALE + WIDTH / 2
                y1 = y1 * self.SCALE + HEIGHT / 2
                updated_orbit.append((x1, y1))
            
            pg.draw.lines(win, self.color, False, updated_orbit)
        
        pg.draw.circle(win, self.color, (x, y), self.radius)
        if not self.sun:
            distance_text = FONT.render(f"{round(self.distance_to_sun/1000)} km", 1, WHITE)
            win.blit(distance_text, (x - distance_text.get_width()/2, y - distance_text.get_height()/2))

    def attraction(self, other):
        distance_x = other.x - self.x
        distance_y = other.y - self.y
        distance = sqrt(distance_x ** 2 + distance_y ** 2)

        if other.sun:
            self.distance_to_sun = distance
        
        force = self.G *  self.mass * other.mass / distance**2
        theta = atan2(distance_y, distance_x)
        force_x = cos(theta) * force
        force_y = sin(theta) * force
        return force_x, force_y

    def update_position(self, plantes):
        total_force_x = total_force_y = 0
        for planet in plantes:
            if self == planet:
                continue
            force_x, force_y = self.attraction(planet)
            total_force_x += force_x
            total_force_y += force_y

        self.x_vel += total_force_x / self.mass * self.TIMESTEP
        self.y_vel += total_force_y / self.mass * self.TIMESTEP
        self.x += self.x_vel * self.TIMESTEP
        self.y += self.y_vel * self.TIMESTEP
        self.orbit.append((self.x, self.y))

def main():
    run = True
    clock = pg.time.Clock()

    sun = Planet(0, 0, 30, YELLOW, 1.98892 * 10**30, 0)
    sun.sun = True
    earth = Planet(-1 * Planet.AU, 0, 16,BLUE, 5.9742 * 10**24, 29.783 * 1000)
    mars = Planet(-1.524 * Planet.AU, 0, 12,RED, 6.39 * 10**23, 24.077 * 1000)
    mercury = Planet(0.387 * Planet.AU, 0, 8,GREYISH, 3.3 * 10**23, -47.4 * 1000)
    venus = Planet(0.723 * Planet.AU, 0, 14, GOLD, 4.8685 * 10**24, -35.02 * 1000)

    plantes = [sun, earth, mars, mercury, venus]

    while run:
        clock.tick(60)
        win.fill((0, 0, 0))
        for event in pg.event.get():
            if event.type == pg.QUIT:
                run = False

        for planet in plantes:
            planet.update_position(plantes)
            planet.draw(win)

        pg.display.update()

    pg.quit()

main()